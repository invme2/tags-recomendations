...
## #005 — Порча pipeline.db (writer-гонка бэбиситтера) + пустые keyword-коллекции
Симптом: (A) после прогона chunk2 файл `runs/health-care-chunk2-2026-06-01/
pipeline.db` стал ровно 1073741824 байт (1 GiB) и `PRAGMA integrity_check`
падает с `database disk image is malformed`. (B) Параллельно: 186 keyword-
коллекций (SEO-лендинги из DataForSEO) были пустыми, ещё 431 — тонкими (1-4
товара), медиана 1 товар/коллекция — слабый сигнал для Google.

Корневая причина: (A) НЕ нехватка диска (свободно 142 ГБ). Бэбиситтер
(`babysit_chunk2.py`) при рестарте делал `Popen` нового раннера, полагаясь
только на `alive()==False`. Полумёртвый предыдущий раннер, дописывавший
большой `final_html`/`scrape_json` blob, ещё держал файл → ДВА процесса
писали один SQLite → порча. Ровно 1 GiB — точка, где конкурентная запись
разорвала B-tree. (B) Пайплайн привязывает товар к коллекции по
валидированным DataForSEO-ключам; коллекции, на чьи ключи не замапился ни
один товар, остаются пустыми.

Решение:
1. `babysit_chunk2.py` → `kill_stragglers()`: перед каждым рестартом
   force-kill любого раннера с этим run-NAME (`Stop-Process -Force`) и блок
   до полного исчезновения процесса (+grace на release WAL-лока).
   SINGLE-WRITER гарантия. Если раннер не умирает — цикл рестарта
   пропускается, гонки не будет.
2. Товары в Shopify не пострадали (БД — лишь локальный чекпойнт; пуш в
   Shopify уже состоялся). chunk2 фактически завершён: 1776 done ≥ 1722
   уникальных товара в CSV (остаток — relisting-дубли, дедуп их скипает).
3. Наполнение коллекций: новый `pipeline/tools/fill_collections.py` —
   TF-IDF матчинг коллекция↔товар (прямой по заголовку/тегам) + кластер
   таксономии как booster ранга (не добавляет внетематическое). Источник
   товаров `--from-shopify` (живой стор, НЕ локальная БД — устойчиво к
   порче). Идемпотентно. Результат: пусто 186→0, медиана 1→6, наполнено
   731/733 авто + 2 вручную. Аудит: `pipeline/tools/collections_census.py`.

Профилактика: для других long-run бэбиситтеров — тот же single-writer
паттерн (kill-before-restart). Битый 1 GiB файл chunk2 можно удалить
(бесполезен), `fill_collections --from-shopify` его и так не читает.
